...
if __name__ == '__main__':
    ...
    print "full distance matrix", mytimeit("a.update(Y,Y)")
    print "pairwise distance matrix", mytimeit("b.update(Y)")
    print "kdTree", mytimeit("e.update(Y,Y)")
    print "nonbonded list", mytimeit("f.update(Y,Y)")
    print "silly", mytimeit("g.update(Y,Y)")
    print "nonbonded pairwise", mytimeit("h.update(Y)")
    
    c = IncompleteDistanceMatrix(a.shape[0], a.shape[1], e.indices)
    c2 = cIncompleteDistanceMatrix(a.shape[0], a.shape[1], e.indices)
    print "incomplete distance matrix", mytimeit("c.update(Y,Y)")
    print "c incomplete distance matrix", mytimeit("c2.update(Y,Y)")
